...
'NEW'({parse,Data}, _From, State) ->
  case Data of
    ...
    _ ->
      ...
      {reply,{waiting,ExistingFrames},'WAITING',#state{frames = [Data|ExistingFrames], uncomplete_data = Data}}
  end.
...
'WAITING'({parse,Data}, _From, State) ->
  ...
  {reply, {waiting, ExistingFrames}, 'WAITING',
    #state{frames = lists:reverse([Data|ExistingFrames]), uncomplete_data = <<UncompleteData/binary,Data/binary>>}
  }.
